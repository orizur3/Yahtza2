...
import { geocodeAddress, computeGeoFields, recomputeAllGeo, Report, GeoStatus, Priority, ReportStatus, PRIORITY_CFG, STATUS_CFG, ALL_STATUSES, ALL_PRIORITIES, CLOSED_STATUSES, Comment, Shift } from '../lib/geo'
...
// Bulk Close Modal
function BulkCloseModal({ count, onConfirm, onClose }: { count: number; onConfirm: (status: ReportStatus) => void; onClose: () => void }) {
    ...
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:14, padding:24, minWidth:320, direction:'rtl' }}>
        ...
        <div style={{ marginBottom:16 }}>
          <label style={{ display:'block', fontSize:13, fontWeight:700, marginBottom:6, color:'#374151' }}>בחר סטטוס סגירה:</label>
          {(['הושלם','נסגר כדיווח שווא'] as ReportStatus[]).map(s => {
            const cfg = STATUS_CFG[s]
            return (
              <div key={s} onClick={() => setStatus(s)} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderRadius:8, border:`2px solid ${status===s?cfg.border:'#e5e7eb'}`, background:status===s?cfg.bg:'#fff', cursor:'pointer', marginBottom:8 }}>
        ...
        <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
          ...
          <button onClick={() => onConfirm(status)} style={{ padding:'8px 20px', borderRadius:7, border:'none', background:'#1d4ed8', color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer' }}>
            סגור {count} אירועים
          </button>
        </div>
      </div>
...
// Merge Modal
function MergeModal({ reports, selectedIds, onConfirm, onClose }: { reports: Report[]; selectedIds: string[]; onConfirm: (keepId: string, mergeId: string) => void; onClose: () => void }) {
  ...
  return (
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:14, padding:24, minWidth:380, direction:'rtl' }}>
        ...
        <p style={{ fontSize:13, color:'#6b7280', marginBottom:16 }}>בחר איזו רשומה תישאר ראשית — השנייה תתמזג אליה</p>
        {selected.map(r => (
          <div key={r.id} onClick={() => setKeepId(r.id)} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 14px', borderRadius:8, border:`2px solid ${keepId===r.id?'#1d4ed8':'#e5e7eb'}`, background:keepId===r.id?'#eff6ff':'#fff', cursor:'pointer', marginBottom:8 }}>
            ...
            <div>
              <div style={{ fontWeight:700, fontSize:14 }}>{r.street}, {r.city}</div>
              <div style={{ fontSize:11, color:'#6b7280' }}>{r.report_content || '—'}</div>
            </div>
          </div>
        ))}
    ...
    <div onClick={onClose} style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', zIndex:2000, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
      <div onClick={e=>e.stopPropagation()} style={{ background:'#fff', borderRadius:14, padding:24, minWidth:360, direction:'rtl' }}>
        ...
        <p style={{ fontSize:13, color:'#6b7280', marginBottom:16 }}>
          {currentShift ? `המטח הנוכחי "${currentShift.name}" ייסגר. אירועים פתוחים יועברו אוטומטית.` : 'פתיחת המטח הראשון'}
        </p>
        ...
        <div style={{ display:'flex', gap:8, marginBottom:16 }}>
          <input value={name} onChange={e=>setName(e.target.value)} placeholder={`${today} — בוקר`}
            style={{ flex:1, padding:'8px 10px', border:'1px solid #d1d5db', borderRadius:7, fontSize:13, direction:'rtl', outline:'none' }}/>
        </div>
        <div style={{ display:'flex', gap:8, justifyContent:'flex-end' }}>
          <button onClick={onClose} style={{ padding:'8px 16px', borderRadius:7, border:'1px solid #d1d5db', background:'#fff', cursor:'pointer', fontSize:13 }}>ביטול</button>
          <button onClick={() => { if(name.trim()||true) onStart(name.trim()||`${today} — מטח`) }} style={{ padding:'8px 20px', borderRadius:7, border:'none', background:'#0f172a', color:'#fff', fontWeight:700, fontSize:13, cursor:'pointer' }}>פתח מטח</button>
        </div>
      </div>
    </div>
  )
...
// Comments Modal
function CommentsModal({ report, onClose, onLogEntry }: { report: Report; onClose: () => void; onLogEntry: (content: string, reportId: string) => void }) {
  ...
  const initMap = useCallback(() => {
    ...
    withCoords.forEach(r => {
      ...
      const info = `<div style="direction:rtl;font-family:Arial;min-width:200px;padding:4px">
        ...
        ${r.maps_url?`<a href="${r.maps_url}" target="_blank" style="font-size:11px;color:#1d4ed8;display:block;margin-top:6px">נווט למיקום →</a>`:''}
      </div>`
...
function exportDailySummary(reports: Report[], shifts: Shift[]) {
  const today = new Date().toLocaleDateString('he-IL')
  const open = reports.filter(r => !CLOSED_STATUSES.includes(r.status) && !r.is_merged)
  const closed = reports.filter(r => CLOSED_STATUSES.includes(r.status))
  const cityCount: Record<string,number> = {}
  reports.filter(r=>!r.is_merged).forEach(r => { cityCount[r.city] = (cityCount[r.city]||0)+1 })
  ...
  <p style="color:#6b7280">הופק ב-${new Date().toLocaleTimeString('he-IL')} | ${shifts.length} מטחים היום</p>
  <div>
    <div class="stat"><div class="stat-n">${reports.filter(r=>!r.is_merged).length}</div><div class="stat-l">סה"כ אירועים</div></div>
  ...
  ${shifts.length>0?`<h2>מטחים היום</h2><table><thead><tr><th>שם מטח</th><th>פתיחה</th><th>סגירה</th><th>אירועים</th></tr></thead><tbody>
    ${shifts.map(s=>`<tr><td><b>${s.name}</b></td><td>${new Date(s.started_at).toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'})}</td><td>${s.ended_at?new Date(s.ended_at).toLocaleTimeString('he-IL',{hour:'2-digit',minute:'2-digit'}):'פעיל'}</td><td>${reports.filter(r=>r.shift_id===s.id).length}</td></tr>`).join('')}
  </tbody></table>`:''}
  <h2>עומס לפי עיר</h2>
...
function ReportForm({initial,onSubmit,onCancel,geocoding}:{initial?:FormData;onSubmit:(f:FormData)=>void;onCancel:()=>void;geocoding:boolean}) {
  ...
  // Auto-set status to כוחות בדרך when unit is assigned
  const handleUnitChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const val = e.target.value
    setForm(f => ({
      ...f,
      assigned_unit: val,
      status: val.trim() && f.status === 'חדש' ? 'כוחות בדרך' : f.status
    }))
  }
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'10px 16px'}}>
        ...
        <div>
          <label style={{display:'block',fontSize:12,fontWeight:700,marginBottom:4,color:'#374151'}}>
            כוח מוקצה
            <span style={{fontSize:10,color:'#6b7280',fontWeight:400,marginRight:4}}>(מעביר לסטטוס "כוחות בדרך" אוטומטית)</span>
          </label>
          <input style={inp} value={form.assigned_unit} onChange={handleUnitChange} placeholder="ניידת 3, יחידה א..."/>
        </div>
...
export default function Page() {
  ...
  const loadData = useCallback(async()=>{
    ...
    if(sh) {
      ...
      setCurrentShift((sh as Shift[]).find(s=>s.is_active) || null)
    }
    setLoading(false)
  },[])
  ...
  useEffect(()=>{
    loadData()
    const ch = supabase.channel('reports-v6')
  ...
  // Auto-set כוחות בדרך when unit assigned inline
  const handleUnitUpdate = async (r: Report, unit: string) => {
    ...
    if(unit.trim()) await addLog(`כוח "${unit}" הוקצה לאירוע ${r.street}, ${r.city}${newStatus==='כוחות בדרך'?' — סטטוס עודכן לכוחות בדרך':''}`, r.id)
    loadData()
  }
  ...
  const handleBulkClose = async (status: ReportStatus) => {
    for(const id of selectedIds) {
      ...
      if(r) await addLog(`אירוע נסגר (${status}): ${r.street}, ${r.city}`, id)
    }
    setSelectedIds([])
    setShowBulkClose(false)
    toast(`${selectedIds.length} אירועים נסגרו בהצלחה`, 'ok')
    loadData()
  }

  const handleMerge = async (keepId: string, mergeId: string) => {
    const keep = reports.find(r=>r.id===keepId)
    const merge = reports.find(r=>r.id===mergeId)
    ...
    const totalCasualties = (keep.casualties||0) + (merge.casualties||0)
    await supabase.from('reports').update({
      street: mergedStreet,
      location_text: `${mergedStreet}, ${keep.city}`,
      casualties: totalCasualties,
      report_content: mergedContent,
    ...
    setSelectedIds([])
    setShowMerge(false)
    toast(`האירועים אוחדו בהצלחה: ${mergedStreet}`, 'ok')
    loadData()
  }

  const handleStartShift = async (name: string) => {
    if(currentShift) {
      await supabase.from('shifts').update({ is_active: false, ended_at: new Date().toISOString() }).eq('id', currentShift.id)
      // Transfer open reports to new shift
    }
    const { data: newShift } = await supabase.from('shifts').insert({
      name, date: new Date().toISOString().split('T')[0], is_active: true, started_at: new Date().toISOString()
    }).select().single()
    if(newShift) {
      ...
      await addLog(`מטח חדש נפתח: "${name}" — ${openReports.length} אירועים פתוחים הועברו`)
      toast(`מטח "${name}" נפתח — ${openReports.length} אירועים הועברו`, 'ok')
    }
  ...
  const handleCreate = async(form:FormData)=>{
    ...
    await addLog(`אירוע חדש נפתח: ${form.street}, ${form.city} | עדיפות: ${form.priority} | סטטוס: ${form.status}${form.assigned_unit?' | כוח: '+form.assigned_unit:''}`, inserted.id)
    if(geoFields.geo_status!=='ללא קשר'){
      toast(`נמצא קשר גיאוגרפי: ${geoFields.geo_summary}`,'warn')
      await addLog(`קשר גיאוגרפי זוהה: ${geoFields.geo_summary} (${form.street}, ${form.city})`, inserted.id)
  ...
  const handleUpdate = async(form:FormData)=>{
    ...
    await addLog(`אירוע עודכן: ${form.street}, ${form.city} | סטטוס: ${form.status} | עדיפות: ${form.priority}`, editReport.id)
  ...
  const handleDelete = async(id:string)=>{
    ...
    for(const rep of recomputed){
      const orig = remaining.find(x=>x.id===rep.id)
      if(orig && (orig.geo_status!==rep.geo_status || JSON.stringify(orig.geo_related_reports)!==JSON.stringify(rep.geo_related_reports))){
        await supabase.from('reports').update({ geo_status:rep.geo_status, geo_related_reports:rep.geo_related_reports, geo_summary:rep.geo_summary, geo_distance_meters:rep.geo_distance_meters }).eq('id',rep.id)
      }
    }
  ...
  const sortedReports = [...reports].filter(r=>!r.is_merged).sort((a,b) => {
  ...
  const filtered = sortedReports.filter(r=>{
    ...
    return (!search||s.includes(search.toLowerCase())) && (!filterGeoStatus||r.geo_status===filterGeoStatus) && (!filterReportStatus||r.status===filterReportStatus) && (!filterPriority||r.priority===filterPriority) && (!filterCity||r.city===filterCity) && (!filterDateFrom||rDate>=new Date(filterDateFrom)) && (!filterDateTo||rDate<=new Date(filterDateTo+'T23:59:59'))
  })

  const cities = reports.filter(r=>!r.is_merged).map(r=>r.city).filter((v,i,a)=>v&&a.indexOf(v)===i)
  const openCount = reports.filter(r=>!CLOSED_STATUSES.includes(r.status)&&!r.is_merged).length
  const criticalCount = reports.filter(r=>r.priority==='דחוף'&&!CLOSED_STATUSES.includes(r.status)&&!r.is_merged).length
  ...
  return (
    <div style={{direction:'rtl',minHeight:'100vh',background:'#f3f4f6',fontFamily:'Arial,sans-serif'}}>
      ...
      {commentsReport && <CommentsModal report={commentsReport} onClose={()=>setCommentsReport(null)} onLogEntry={(content,reportId)=>addLog(content,reportId)}/>}
      ...
      <div style={{background:'#0f172a',padding:'10px 22px',display:'grid',gridTemplateColumns:'1fr auto 1fr',alignItems:'center'}}>
        <div style={{display:'flex',gap:6,alignItems:'center',flexWrap:'wrap'}}>
          ...
          <button onClick={()=>exportDailySummary(reports,shifts)} style={{background:'#3b0764',color:'#d8b4fe',border:'1px solid #7e22ce',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>📋 סיכום</button>
        ...
        <div style={{display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
          ...
          {currentShift && <div style={{background:'#1e3a5f',borderRadius:20,padding:'2px 10px',fontSize:11,color:'#93c5fd'}}>מטח: {currentShift.name}</div>}
        </div>
        <div style={{display:'flex',justifyContent:'flex-end',gap:8,alignItems:'center'}}>
          <button onClick={()=>setShowShiftModal(true)} style={{background:'#1e293b',color:'#94a3b8',border:'1px solid #334155',borderRadius:8,padding:'7px 12px',fontSize:12,cursor:'pointer'}}>
            {currentShift?'🔄 מטח חדש':'➕ פתח מטח'}
          </button>
      ...
      <div style={{background:'#1e293b',padding:'8px 22px',display:'flex',gap:18,flexWrap:'wrap'}}>
        {[
          {n:reports.filter(r=>!r.is_merged).length,l:'סה"כ',c:'#60a5fa'},
          {n:reports.filter(r=>r.priority==='דחוף'&&!r.is_merged).length,l:'דחוף',c:'#f87171'},
          {n:reports.filter(r=>r.priority==='גבוה'&&!r.is_merged).length,l:'גבוה',c:'#f97316'},
          {n:openCount,l:'פתוחים',c:'#a78bfa'},
          {n:reports.filter(r=>r.geo_status!=='ללא קשר'&&!r.is_merged).length,l:'קשר גיאו',c:'#fac775'},
      ...
      <div style={{padding:16}}>
        ...
        <div style={{background:'#fff',borderRadius:10,padding:'10px 14px',marginBottom:12,border:'1px solid #e5e7eb'}}>
          <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
            ...
            <button onClick={()=>setShowAdvSearch(!showAdvSearch)} style={{...inp,background:showAdvSearch?'#eff6ff':'#fff',color:showAdvSearch?'#1d4ed8':'#374151',cursor:'pointer'}}>🔍 מתקדם {showAdvSearch?'▲':'▼'}</button>
              ...
              <table style={{width:'100%',borderCollapse:'collapse',minWidth:1100}}>
                <thead>
                  <tr>
                    ...
                    {['עדיפות','עיר','רחוב','שעה','סטטוס','כוח','נפגעים','תוכן','🔴 גיאו','🧠 פירוט','קשור','💬','נווט','פעולות'].map(h=>(
                ...
                <tbody>
                  {filtered.map(r=>{
                    ...
                    const rowBg = isSelected?'#eff6ff':r.priority==='דחוף'&&!CLOSED_STATUSES.includes(r.status)?'#fff5f5':focusReport?.id===r.id?'#f0f9ff':r.geo_status==='מצטלבים'?'#fff9f9':'transparent'
                    const pc = PRIORITY_CFG[r.priority||'רגיל']
                    const sc = STATUS_CFG[r.status||'חדש']
                    ...
                    return(
                      <tr key={r.id} style={{background:rowBg}}>
                        ...
                        <td style={cell} onClick={()=>focusOnMap(r)}><b>{r.city}</b></td>
                        <td style={cell} onClick={()=>focusOnMap(r)}>{r.street}</td>
                        ...
                        <td style={{...cell,maxWidth:100}} onClick={e=>e.stopPropagation()}>
                          <input
                            defaultValue={r.assigned_unit||''}
                            onBlur={e=>{ if(e.target.value!==r.assigned_unit) handleUnitUpdate(r,e.target.value) }}
                            onKeyDown={e=>{ if(e.key==='Enter') (e.target as HTMLInputElement).blur() }}
                            placeholder="הוסף כוח..."
                        ...
                        <td style={{...cell,maxWidth:120}} onClick={()=>focusOnMap(r)}><span style={{display:'block',overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',maxWidth:110,fontSize:11}}>{r.report_content||<span style={{color:'#d1d5db'}}>—</span>}</span></td>
                        ...
                        <td style={{...cell,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
                          <a href={navUrl} target="_blank" rel="noreferrer" style={{display:'inline-block',background:'#eff6ff',color:'#1d4ed8',border:'1px solid #bfdbfe',borderRadius:7,padding:'3px 7px',fontSize:11,textDecoration:'none',fontWeight:600}}>🧭 נווט</a>
                        </td>
                        <td style={{...cell,textAlign:'center'}} onClick={e=>e.stopPropagation()}>
